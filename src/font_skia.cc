...
static void FontSkia_SetSkiaPaint(sk_sp<SkTypeface> typeface, SkPaint* skPaint, const android::MinikinPaint& paint) {
...
void FontSkia::GetBounds(android::MinikinRect* bounds, uint32_t glyph_id,
    const android::MinikinPaint& paint) const {
    SkPaint skPaint;
    uint16_t glyph16 = glyph_id;
    SkRect skBounds;
...
const void* FontSkia::GetTable(uint32_t tag, size_t* size, android::MinikinDestroyFunc* destroy) {
